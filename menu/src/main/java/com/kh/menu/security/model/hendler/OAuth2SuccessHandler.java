package com.kh.menu.security.model.hendler;
...
import com.kh.menu.security.model.dto.customOAyth2User;
...
public class OAuth2SuccessHandler implements AuthenticationSuccessHandler {
	...
	// 클라이언트에게 access, refresh토큰 등을 발급
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		customOAyth2User oauthUser = (customOAyth2User) authentication.getPrincipal();
		...
		String roles = oauthUser.getAuthorities()
								.stream()
								.map(GrantedAuthority::getAuthority)
								.collect(Collectors.joining("|"));
		
		ResponseCookie accessCookie = CookieUtil.createTokenCookie(CookieUtil.ACCESS_COOKIE, accessToken.toString(), 30);
		ResponseCookie refreshCookie = CookieUtil.createTokenCookie(CookieUtil.REFRESH_COOKI, refreshToken.toString(), 30);
