...
public class OAuth2Service implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
	 ...
	 *    개방형 표준 프로토콜
	 *  - 사용자는 제3자의 앱에서 제공하는 인증페이지에서 인증을 진행하고, 인가권한을
	 *    백엔드서버에 위임하여 제3자앱의 비밀번호를 백엔드에 전달하지 않고, 특정 서비스의
	 *    정보를 백엔드에서 이용할 수 있게 해준다.
	 *  - OAuth2프로토콜 사용시 사용자는 자신의 앱 비밀번호가 노출되지 않기 때문에,
	 *    클라이언트 서버가 해킹당하더라도 안전하며, 매번 새로운 서비스에 가입할 필요없이
	 ...
	 * DB에서 사용자 정보 조회 후,
	 * 존재하지 않는 사용자라면 자동 회원가입, 존재하는 회원이라면 로그인
	 */
